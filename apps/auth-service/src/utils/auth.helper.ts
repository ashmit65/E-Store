...
export const checkOtpRestricitons = async (email: string) => {
...
export const sendOtp = async (name: string, email: string, template: string) => {
    ...
    const activationLink = `http://localhost:3000/activation/${otp}`;

    await sendMail(email, "Verify Your Email", template, { name, otp, activationLink });
...
export const verifyOtp = async (email: string, otp: string) => {
    ...
    if (storedOtp !== otp) {
        if (failedAttempts >= 2) {
            ...
            throw new ValidationError("Too many failed attempts. Please try again after 30 minutes");
        }
...
export const handleForgetPassword = async (req: Request, res: Response, next: NextFunction, userType: 'buyer' | 'seller') => {
    try{
        ...
        const user = userType === 'buyer' && await prisma.users.findUnique({where:{email}});
...
export const verifyForgetPassowrdOtp = async(req: Request, res: Response, next: NextFunction) => {
    try {
